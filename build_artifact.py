...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-luaexpat-*.xml'))
if len(astrMatch) == 0:
    raise Exception('No match found for "lua5.4-luaexpat-*.xml".')
elif len(astrMatch) > 1:
    raise Exception('More than one match found for "lua5.4-luaexpat-*.xml". Clean your build folder.')
